...
// Self-contained high-performance Vector2 class to bypass 'vecteur' missing package risks
class Vec2 {
...
export const MagneticCursor: FC<MagneticCursorProps> = ({
  ...
  useEffect(() => {
    ...
    const onMouseMove = (event: PointerEvent) => {
      ...
      gsap.to(cursorEl, { opacity: isInContainer ? 1 : 0, duration: 0.2, overwrite: 'auto' });
    ...
    const initializeMagneticHandlers = () => {
    ...
    // Initialize magnetic links after UI completes render
    const timeoutId = setTimeout(initializeMagneticHandlers, 100);
  ...
  return (
    <div ref={containerRef} className="relative w-full h-full cursor-none">
      <div id="magnetic-cursor-el" ref={cursorRef} className={`magnetic-cursor ${cursorClassName}`} style={styles} />
      {children}
    </div>
  );
};
